...
ytplay.getFirstSearch = async (name) => {
    name=name.toString().replace(/,/gi,"+");
    let html = await html_fetch.fetch("GET", "https://www.youtube.com/results?search_query="+name);
    
    let videoId = await JSON_Search.findJSON(
        "search.html",
        "VideoId",
        html,
        utils.regx.contents,
        "</script>",        //not the actual right but works for now (in progress)
        "{"
        ).contents;

    let i=0;
    while(videoId[i].searchPyvRenderer&&i<videoId.length){
        i++;
    }
    videoId=videoId[i].videoRenderer.videoId;
    
    let link = utils.BASE_URL+videoId;
...
ytplay.getInfo = async (url) => {
    ...
    info.videoDetails.video_url=url;
